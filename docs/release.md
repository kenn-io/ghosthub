...
```bash
shasum -a 256 -c Ghosthub_0.5.2_macos_arm64.dmg.sha256
...
- About Ghosthub reports version 0.5.2, Kenn Software LLC copyright, and the
...
The script creates and pushes annotated tag `v0.5.2`. The tag workflow rebuilds
from source, repeats signing and notarization, generates the production-signed
appcast, and creates
`https://github.com/kenn-io/ghosthub/releases/tag/v0.5.2` with:

- `Ghosthub_0.5.2_macos_arm64.dmg`
- `Ghosthub_0.5.2_macos_arm64.dmg.sha256`
- `SHA256SUMS`
- `appcast.xml`
...
KWT_BINARY_PATH=/absolute/path/to/kwt \
make release-dmg RELEASE_APP_VERSION=0.5.2
```
...
make release-appcast RELEASE_APP_VERSION=0.5.2
```
