...
extension WorkspaceZellijTests {
    ...
    @Test("reconnect reacquires a newer lease for the same SSH route")
    func reconnectReacquiresNewerLeaseForSameSSHRoute() async throws {
        let database = try WorkspaceDatabase.inMemory()
        let host = HostSummary(
            id: UUID(),
            configKey: "builder",
        ...
        let changed = SSHConnectionArgumentsSnapshot(testKwtSSHAttachment(
            arguments: ["-F", "/tmp/changed-zellij-config"],
            routeIdentity: "sha256:stable-route",
            generation: 2
        ))
